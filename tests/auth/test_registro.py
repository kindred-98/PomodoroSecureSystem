...
class TestRegistroValidacion:
    ...
    def test_rol_invalido(self, mock_conexion_global, fernet_key_env, parametros_contraseña_defecto):
        with pytest.raises(ValueError, match="rol debe ser uno de"):
            registrar_usuario("a@b.com", "Nombre", "admin", parametros_contraseña_defecto)
...
class TestRegistroExito:
    ...
    def test_email_duplicado_falla(self, mock_conexion_global, fernet_key_env, parametros_contraseña_defecto):
        ...
        with pytest.raises(Exception, match="ya está registrado"):
            registrar_usuario("dup@test.com", "Segundo", "empleado", parametros_contraseña_defecto)
    
    def test_todos_los_roles(self, mock_conexion_global, fernet_key_env, parametros_contraseña_defecto):
        """Debe aceptar los 3 roles válidos"""
        for rol in ["empleado", "encargado", "supervisor"]:
            email = f"{rol}@test.com"
            resultado = registrar_usuario(email, rol.title(), rol, parametros_contraseña_defecto)
            assert resultado['usuario']['rol'] == rol
